...
function sessionsDir(projectHash) {
    return join(projectDir(projectHash), "sessions");
}
function indexPath(projectHash) {
    return join(projectDir(projectHash), "index.json");
}
function sessionPath(projectHash, sessionId) {
 ...
 */
export function timelineProjectHash(projectPath) {
    return sessionIdFromPath(projectPath);
}
/**
 * Save a timeline session to disk.
 */
export async function saveTimelineSession(session) {
    const dir = sessionsDir(session.projectHash);
    await ensureDir(dir);
    await writeFile(sessionPath(session.projectHash, session.id), JSON.stringify(session, null, 2));
}
/**
 * Load a timeline session by ID.
 */
export async function loadTimelineSession(projectHash, sessionId) {
    try {
 ...
 */
export async function loadTimelineIndex(projectHash) {
    try {
 ...
 */
export async function getCapturedIds(projectHash) {
 ...
 */
export async function addToIndex(projectHash, entry) {
 ...
 */
export async function updateIndexEntry(projectHash, sessionId, updates) {
    const index = await loadTimelineIndex(projectHash);
 ...
 */
export async function ensureTimelineStorage() {
