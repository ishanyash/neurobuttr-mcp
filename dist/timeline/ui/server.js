...
import { loadTimelineIndex, loadTimelineSession, timelineProjectHash, getCapturedIds, saveTimelineSession, addToIndex, sessionToIndexEntry, } from "../storage.js";
import { claudeProjectHash, findSessionFiles, filterUncaptured, parseJSONLFile, } from "../parser.js";
const __dirname = dirname(fileURLToPath(import.meta.url));
export async function startTimelineServer(projectPath, port = 3100) {
    const projHash = timelineProjectHash(projectPath);
    const sessionsDir = join(homedir(), ".neurobuttr", "timeline", projHash, "sessions");
        ...
        watch(sessionsDir, { persistent: false }, () => {
            ...
            debounceTimer = setTimeout(() => {
                for (const client of sseClients) {
                    client.res.write(`event: session-update\ndata: ${JSON.stringify({ type: "update" })}\n\n`);
                }
            }, 500);
        });
    }
    ...
    server.on("error", (err) => {
        if (err.code === "EADDRINUSE") {
            console.error(`Port ${port} is already in use. Try: npx neurobuttr-mcp ui --port ${port + 100}`);
            process.exit(1);
        }
        throw err;
    });
...
async function autoCapture(projectPath, projHash) {
    try {
