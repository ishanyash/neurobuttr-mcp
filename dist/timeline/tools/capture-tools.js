import { z } from "zod";
import { claudeProjectHash, findSessionFiles, filterUncaptured, parseJSONLFile, } from "../parser.js";
import { timelineProjectHash, getCapturedIds, saveTimelineSession, addToIndex, sessionToIndexEntry, } from "../storage.js";
export function registerTimelineCaptureTools(server) {
    server.tool("nb_timeline_capture", `Parse Claude Code session logs and capture them as structured timeline records. Run this after a Claude Code session to record what the agent did — prompts, tool calls, reasoning, and file diffs. Use all=true to capture all uncaptured sessions.`, {
        cwd: z
            .string()
            .optional()
        ...
        try {
            ...
            for (const filePath of uncaptured) {
                ...
                if (session) {
                    ...
                    captured.push({
                        id: session.id,
                        prompt: session.prompt.length > 80
...
//# sourceMappingURL=capture-tools.js.map