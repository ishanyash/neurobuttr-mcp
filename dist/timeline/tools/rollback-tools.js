...
import { timelineProjectHash, loadTimelineIndex, loadTimelineSession, saveTimelineSession, updateIndexEntry, } from "../storage.js";
function execGit(command, cwd) {
    return execSync(`git ${command}`, { cwd, encoding: "utf-8" }).trim();
}
export function registerTimelineRollbackTools(server) {
    server.tool("nb_timeline_rollback", `Revert all changes from a captured Claude Code session. Creates a git revert commit if the session's changes were committed, or reverse-applies diffs for uncommitted changes. Requires a clean working tree.`, {
        session_id: z
            .string()
        ...
        try {
            ...
            if (session.gitCommitAfter) {
                try {
                    ...
                    execGit(`merge-base --is-ancestor ${session.gitCommitAfter} HEAD`, projectPath);
                    // Create revert commit
                    execGit(`revert --no-edit ${session.gitCommitAfter}`, projectPath);
...
//# sourceMappingURL=rollback-tools.js.map