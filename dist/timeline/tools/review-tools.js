...
function formatAction(action) {
    ...
    switch (action.type) {
        case "reasoning":
            ...
            lines.push(action.reasoning.length > 300
        ...
        case "tool_use":
            ...
            if (action.toolName === "Edit" || action.toolName === "Write") {
                ...
                lines.push(`  $ ${cmd.length > 100 ? cmd.slice(0, 100) + "..." : cmd}`);
            }
                ...
                lines.push(`  input: ${summary.length > 120 ? summary.slice(0, 120) + "..." : summary}`);
            }
            if (action.fileDiff) {
            ...
            break;
        case "tool_result":
            lines.push(`[${action.index}] ${action.isError ? "❌" : "✅"} Result (${ts})`);
...
export function registerTimelineReviewTools(server) {
    // nb_timeline_list
    server.tool("nb_timeline_list", `List captured Claude Code sessions for this project. Shows prompt, date, model, files changed, and status for each session.`, {
        cwd: z
            .string()
            .optional()
        ...
        try {
            ...
            // Apply file filter
            if (file_filter) {
                sessions = sessions.filter((s) => s.filesModified.some((f) => f.includes(file_filter)));
            }
            // Apply limit
            const max = limit || 10;
            sessions = sessions.slice(0, max);
            const lines = [];
            lines.push(`${index.sessions.length} session(s) captured (showing ${sessions.length}):\n`);
            for (const s of sessions) {
                ...
                lines.push(`${s.id.slice(0, 8)}  ${date}  ${s.model}  ${s.filesModified.length} files  ${s.actionCount} actions${status}`);
                lines.push(`  "${prompt}"`);
    ...
    // nb_timeline_show
    server.tool("nb_timeline_show", `Show detailed information about a captured Claude Code session — the full action timeline with reasoning, tool calls, and file diffs.`, {
        session_id: z
            .string()
        ...
        try {
            ...
            lines.push(`Duration: ${formatDuration(session.startedAt, session.completedAt)}`);
            ...
            lines.push("");
            lines.push(`--- Actions ${startIdx}-${Math.min(endIdx - 1, session.actions.length - 1)} of ${session.actions.length} ---`);
            lines.push("");
            ...
            if (session.subagentSessions.length > 0) {
                ...
                for (const sub of session.subagentSessions) {
                    const subPrompt = sub.prompt.length > 60
                        ? sub.prompt.slice(0, 60) + "..."
                        : sub.prompt;
                    lines.push(`  ${sub.id.slice(0, 8)}: "${subPrompt}" (${sub.actions.length} actions)`);
                }
            }
            return {
                content: [{ type: "text", text: lines.join("\n") }],
    ...
    // nb_timeline_diff
    server.tool("nb_timeline_diff", `Show all file changes in a captured session as unified diffs. Like viewing a PR diff — see exactly what the agent changed.`, {
        session_id: z
            .string()
            .describe('Session ID (full UUID or first 8 chars)'),
        cwd: z
        ...
        try {
            ...
            const lines = [];
            lines.push(`Session ${session_id.slice(0, 8)} — ${byFile.size} file(s) changed:\n`);
 ...
 */
async function resolveSession(projectHash, sessionId) {
