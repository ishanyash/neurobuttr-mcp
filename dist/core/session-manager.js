...
export async function resolveSession(projectPath) {
...
function generateBranchName(anchorContext, existingNames) {
...
function findBranch(session, ref) {
    return (session.branches.find((b) => b.name === ref) ||
        session.branches.find((b) => b.id === ref));
}
// --- Message operations ---
export async function addMessage(sessionId, role, content, branchId, actionType) {
...
// --- Branch operations ---
export async function createBranch(sessionId, anchorContext, parentMessageId, parentBranchId, branchName, anchorSnippet) {
    const session = await getOrCreateSession(sessionId);
    // Auto-pick the last message if none specified
    const resolvedParentMsgId = parentMessageId || session.messages[session.messages.length - 1]?.id || "";
...
export async function checkoutBranch(sessionId, branchRef // name, id, or "main"
) {
    ...
    if (!branch)
        throw new Error(`Branch "${branchRef}" not found. Use nb_log to see available branches.`);
    session.currentBranchId = branch.id;
    session.updatedAt = now();
    await saveSession(session);
    notifyBridge({
...
export async function getBranchContext(sessionId, branchId) {
    const session = await loadSession(sessionId);
    if (!session)
        throw new Error(`Session ${sessionId} not found`);
    const branch = findBranch(session, branchId);
...
export async function resolveBranch(sessionId, branchId, merge = false) {
    const session = await loadSession(sessionId);
    if (!session)
    ...
    if (merge) {
        ...
        if (insightCount > 0) {
            const summaryParts = branchInsights.map((i) => i.content.length > 100 ? i.content.slice(0, 100) + "..." : i.content);
            const summaryMsg = {
                id: generateId(),
                role: "assistant",
...
// --- Insight operations ---
export async function rememberInsight(sessionId, branchId, content) {
    const session = await loadSession(sessionId);
    if (!session)
        throw new Error(`Session ${sessionId} not found`);
    const ref = branchId || session.currentBranchId;
    if (!ref)
        throw new Error("No branch specified and not on any branch.");
...
function buildScopedContext(session, branch) {
    let parentMessages;
    if (branch.parentBranchId) {
        const parentBranch = session.branches.find((b) => b.id === branch.parentBranchId);
    ...
    if (branch.parentMessageId) {
        const parentIndex = parentMessages.findIndex((m) => m.id === branch.parentMessageId);
        if (parentIndex >= 0) {
