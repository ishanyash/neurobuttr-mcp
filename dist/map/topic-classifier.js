...
const MULTI_WORD_SORTED = Object.keys(MULTI_WORD_TERMS).sort((a, b) => b.length - a.length);
...
export function extractNormalizedKeywords(text) {
    return extractFromText(text)
        .sort((a, b) => b.score - a.score)
        .slice(0, 3)
        .map((m) => m.term);
}
export function extractWeightedKeywords(sources) {
    const merged = {};
...
export function classifyTopic(text, anchorContext) {
    if (anchorContext) {
...
export function normalizeTopicsAcrossNodes(nodeKeywords) {
    const globalFreq = {};
    ...
    for (const kw of Object.keys(globalFreq)) {
        const isDictMatch = kw.charAt(0) === kw.charAt(0).toUpperCase() ||
            kw.includes("/") ||
            kw.includes(".");
        if (isDictMatch)
            continue;
...
//# sourceMappingURL=topic-classifier.js.map