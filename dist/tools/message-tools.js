import { z } from "zod";
import { resolveSession, addMessage, rememberInsight, getInsights, } from "../core/session-manager.js";
export function registerMessageTools(server) {
    // nb_add — add a message to the current branch or main thread
    server.tool("add_message", "Add a message to the current branch or main thread. Used internally to track conversation history.", {
    ...
    server.tool("remember_insight", `Save a key finding or insight from a branch exploration back to the main thread. Like "git stash" but for knowledge — insights persist and are visible from any branch. Use this when you discover something valuable during a side exploration.`, {
    ...
    server.tool("get_insights", "List all remembered insights across all branches. Shows what was discovered during explorations.", {
        cwd: z
            .string()
            .optional()
            .describe("Project directory (auto-detected from environment if omitted)"),
        ...
        try {
            ...
            if (insights.length === 0) {
                return {
                    content: [
                        { type: "text", text: "No insights saved yet. Use remember_insight to save findings from branches." },
                    ],
                };
            }
