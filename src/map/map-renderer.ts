...
export function renderMermaid(
  ...
  for (const node of nodes) {
    ...
    if (node.type === "spine") {
      ...
      lines.push(
        `  ${node.id}["${label}<br/>${msgCount}${topic}${snippetTag}${suffix}"]`
      );
    }
  }
...
export function renderAscii(
  ...
  const { nodes, edges, stats } = graph;
  if (nodes.length === 0) return "No data yet.";

  const lines: string[] = [];
  lines.push(
    `Main Thread (${stats.mainMessages} messages, ${stats.branches} branches)`
  );
  ...
  if (insights.length > 0) {
    lines.push("");
    lines.push(
      `Insights (${insights.length}): ${insights.map((i) => `"${truncate(i.content, 40)}"`).join(", ")}`
    );
  }
...
function renderBranchChildren(
  ...
  for (let i = 0; i < childIds.length; i++) {
    ...
    if (!node || node.type === "spine") continue;
    ...
    lines.push(
      `${indent}${prefix} 🔀 ${label} (${node.messageCount} msgs${resolved})${topic}`
    );
...
export function renderMap(
  graph: GraphData,
  format: "mermaid" | "ascii" | "json" = "ascii",
  insights: InsightData[] = []
): string {
  switch (format) {
    case "mermaid":
