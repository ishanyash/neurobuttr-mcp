...
import type { Session, Branch, Message } from "../core/types.js";
import {
  extractNormalizedKeywords,
  extractWeightedKeywords,
  normalizeTopicsAcrossNodes,
...
function computeDepth(branch: Branch, branches: Branch[]): number {
  if (!branch.parentBranchId) return 1;
  const parent = branches.find((b) => b.id === branch.parentBranchId);
  if (!parent) return 1;
  return computeDepth(parent, branches) + 1;
}

// Group main thread messages into user→assistant turns
interface Turn {
...
export function buildGraph(session: Session): {
  nodes: MapNode[];
  edges: MapEdge[];
  stats: {
    mainMessages: number;
  ...
  const nodes: MapNode[] = [];
  const edges: MapEdge[] = [];
  const nodeKeywordsList: { nodeId: string; keywords: string[] }[] = [];

  // --- Build spine from main thread ---
  const turns = groupIntoTurns(session.messages);
  const messageToSpine = new Map<string, string>();
  ...
  turns.forEach((turn, index) => {
    ...
    if (turn.userMessage) messageToSpine.set(turn.userMessage.id, spineId);
