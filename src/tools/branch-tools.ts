...
export function registerBranchTools(server: McpServer): void {
  ...
  server.tool(
    "nb_branch",
    `Create a new conversation branch to explore a tangent (like "git checkout -b"). When the user selects specific text to explore, pass it as anchor_snippet. Automatically branches from the current point unless parent_message_id is specified.`,
    {
      topic: z
        .string()
  ...
  server.tool(
    "nb_checkout",
    `Switch to a different branch or back to main (like "git checkout"). Use "main" to return to the main conversation thread. Use a branch name to switch to an existing branch.`,
    {
      branch: z
        .string()
  ...
  server.tool(
    "nb_resolve",
    `Mark the current branch as resolved/completed and switch back to main (like finishing a feature branch). Optionally specify a branch name to resolve a different branch.`,
    {
      branch: z
        .string()
        .optional()
  ...
  server.tool(
    "nb_context",
    `View the full conversation context for a specific branch, including parent context and branch messages.`,
    {
      branch: z
        .string()
