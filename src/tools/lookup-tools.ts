...
function jsonlPath(projectPath: string, sourceSessionId: string): string {
...
export function registerLookupTools(server: McpServer): void {
  server.tool(
    "nb_lookup",
    `Look up a neurobuttr event by its key (e.g. "nb:a3f2c8" or just "a3f2c8"). Returns the event details and the JSONL file path so you can read the raw source data for full context. Use this when a user references an event key from the metro map UI.`,
    {
      key: z
        .string()
        .describe(
          'Event key from the metro map (e.g. "nb:a3f2c8" or "a3f2c8")'
        ),
      cwd: z
        .string()
        .optional()
    ...
    async ({ key, cwd }) => {
      try {
        ...
        lines.push(
          `**Time:** ${new Date(found.timestamp).toISOString()}`
        );
        ...
        const chatSession = (session.chatSessions || []).find(
          (cs) => cs.id === found!.sourceSessionId
        );
        if (chatSession) {
        ...
        if (found.sourceSessionId) {
          ...
          if (found.sourceEntryUuid) {
            lines.push(
              `**Entry UUID:** \`${found.sourceEntryUuid}\``
            );
            lines.push(
              `To see the full raw data, read the JSONL file and search for this UUID.`
            );
          }
        }
