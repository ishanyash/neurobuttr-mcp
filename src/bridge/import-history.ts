...
import type { Session, Message, Branch, ChatSession, ActionType } from "../core/types.js";
import { createHash } from "node:crypto";
import { saveSession } from "../core/storage.js";
import { generateId, now } from "../utils.js";

/** Generate a short stable event key from a JSONL entry UUID + timestamp */
function makeEventKey(entryUuid: string | undefined, timestamp: number, index: number): string {
...
function toolToActionType(toolName: string): ActionType {
  ...
  if (name.startsWith("mcp_") || name.startsWith("mcp__")) return "other";
  return "other";
}

/** Does this tool typically require user approval in default permission mode? */
function toolRequiresApproval(toolName: string): boolean {
  const name = toolName.toLowerCase();
  ...
  if (name === "notebookedit" || name === "webfetch" || name === "websearch") return true;
  // Read-only tools are auto-approved
  if (name === "read" || name === "grep" || name === "glob") return false;
  // Skill tool doesn't require approval
  if (name === "skill" || name === "toolsearch" || name === "todowrite") return false;
  // MCP tools typically require approval
  if (name.startsWith("mcp_") || name.startsWith("mcp__")) return true;
  return false;
}
  ...
  switch (toolName) {
    ...
    case "TodoWrite":
      if (Array.isArray(input.todos)) {
        const items = input.todos as Array<{ content?: string; status?: string }>;
    ...
    default:
      ...
      if (input && typeof input === "object") {
        ...
        if (firstStr) return `${toolName}: ${firstStr.slice(0, 80)}`;
      }
      return toolName;
  }
}
...
async function findAllSessions(projectPath: string): Promise<string[]> {
 ...
 */
async function findSubagentFiles(projectPath: string, sessionFile: string): Promise<string[]> {
...
function extractMessages(entries: JournalEntry[]): Message[] {
    ...
    if (entry.type === "user") {
      ...
      if (!blocks.some((b) => b.type === "tool_use")) {
        const textBlock = blocks.find(
          (b) => b.type === "text" && b.text && b.text.length > 20
        );
        if (textBlock?.text) {
          const key = makeEventKey(entryUuid, ts, msgIndex++);
          messages.push({
  ...
  for (let idx = 0; idx < sessionFiles.length; idx++) {
    ...
    for (const subFile of subagentFiles) {
      try {
        ...
        for (const m of messages) {
          if (m.timestamp <= subStart) parentMsg = m;
          else break;
        }
