 ...
 */
export function claudeProjectHash(cwd: string): string {
 ...
 */
async function isActiveSession(filePath: string): Promise<boolean> {
  try {
...
export async function parseJSONLFile(
  ...
  for (const msg of messages) {
    ...
    for (const block of content) {
      ...
      if (
        (msg.type === "user" && block.type === "tool_result") ||
  ...
  const subagentSessions = await parseSubagents(
    filePath,
    sessionId,
    projectPath,
    projectHash
