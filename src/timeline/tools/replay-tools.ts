...
export function registerTimelineReplayTools(server: McpServer): void {
  server.tool(
    "nb_timeline_replay",
    `Replay a captured session up to a specific action index. Shows actions and reasoning up to that point, plus a context summary you can use to start a new Claude Code session from that state (lightweight branching). Use this to try a different approach from a known point in a previous session.`,
    {
      session_id: z
        .string()
    ...
    async ({ session_id, at_action, cwd }) => {
      try {
        ...
        const filesModifiedUpTo = new Set<string>();
        const filesReadUpTo = new Set<string>();
        ...
        for (const action of actionsUpTo) {
          ...
          if (action.type === "tool_use") {
            toolCalls.push(
              `${action.toolName}(${JSON.stringify(action.toolInput).slice(0, 80)})`
            );
            if (action.fileDiff) {
              filesModifiedUpTo.add(action.fileDiff.path);
            }
            const fp = action.toolInput.file_path as string | undefined;
            if (fp) {
        ...
        lines.push(
          `To try a different approach from this point, start a new Claude Code session with this context:`
        );
        lines.push("");
        lines.push(`"I previously worked on: ${session.prompt}`);
        lines.push(
          `The agent reached action ${at_action} and had modified: ${[...filesModifiedUpTo].join(", ") || "no files"}.`
        );
        lines.push(
          `Now I want to try a different approach from that point. Here's what was done so far: ${toolCalls.slice(0, 5).join(", ")}${toolCalls.length > 5 ? "..." : ""}`
        );
